...
# =============================================================================
# NIRUCON Debian 13 dwm Postinstall v2.5.0
# =============================================================================
#
# Target:
#   Debian 13 / Trixie
...
print_header() {
  ...
  echo
  echo "============================================================"
  echo "  NIRUCON Debian 13 dwm Postinstall v2.5.0"
  echo "============================================================"
  echo
...
initialize_umc1820_if_present() {
  ...
  if command -v wpctl >/dev/null 2>&1; then
    local sink source
    sink="$(wpctl status 2>/dev/null | awk '/Sinks:/ {sec="sink"; next} /Sources:/ {sec="source"; next} /Filters:/ {sec=""} sec=="sink" && /UMC1820/ {gsub(/[^0-9]/,"",$1); print $1; exit}' || true)"
    source="$(wpctl status 2>/dev/null | awk '/Sources:/ {sec="source"; next} /Filters:/ {sec=""} sec=="source" && /UMC1820/ {gsub(/[^0-9]/,"",$1); print $1; exit}' || true)"
    [[ -n "${sink:-}" ]] && wpctl set-default "$sink" 2>/dev/null || true
    [[ -n "${source:-}" ]] && wpctl set-default "$source" 2>/dev/null || true
  fi
...
echo "   For UMC1820 recording: Audio system = ALSA, input/output = hw:UMC1820."
echo "   Use JACK/PipeWire JACK only when you specifically need inter-application routing."
EOF
...
EOF
  ...
  chmod +x "$LOCAL_BIN/audio-performance.sh" "$LOCAL_BIN/audio-balanced.sh" "$LOCAL_BIN/audio-status.sh" "$LOCAL_BIN/reaper-audio-check.sh" "$LOCAL_BIN/yabridge-sync.sh" "$LOCAL_BIN/umc1820-init.sh" "$LOCAL_BIN/reaper-umc1820-alsa.sh" "$LOCAL_BIN/daw-doctor.sh"
...
if [[ "$INSTALL_AUDIO_TOOLS" -eq 1 ]]; then
  ...
  echo "  daw-doctor.sh           # full DAW health check: realtime, UMC1820, ALSA, REAPER"
...
echo "  Recommended first test: run daw-doctor.sh. For UMC1820, set Reaper audio system to ALSA and choose hw:UMC1820."
echo
ok "Done."

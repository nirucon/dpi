...
# =============================================================================
# NIRUCON Debian 13 dwm Postinstall v2.6.0
# =============================================================================
#
# Target:
#   Debian 13 / Trixie
...
print_header() {
  ...
  echo
  echo "============================================================"
  echo "  NIRUCON Debian 13 dwm Postinstall v2.6.0"
  echo "============================================================"
  echo
...
# -----------------------------------------------------------------------------
# Professional audio hardware helpers
# -----------------------------------------------------------------------------
...
set_amixer_control_if_present() {
  local card="$1" control="$2" value="$3"

  if amixer -c "$card" sget "$control" >/dev/null 2>&1; then
    say "Setting ALSA mixer control on card $card: $control -> $value"
    amixer -c "$card" sset "$control" "$value" >/dev/null 2>&1 || \
      warn "Could not set ALSA mixer control: $control"
  fi
}

initialize_umc1820_if_present() {
  local card channels rate_test
  card="$(find_alsa_card_by_name 'UMC1820|BEHRINGER.*UMC1820' || true)"

  if [[ -z "${card:-}" ]]; then
    note "UMC1820 not detected. Skipping UMC1820 mixer initialization."
    return 0
  fi

  phase "Initializing Behringer UMC1820 ALSA mixer defaults"
  ok "UMC1820 detected as ALSA card $card."

  # Some Debian installs expose the UMC1820 capture mixer at 0 (-127 dB),
  # which makes REAPER/ALSA capture appear dead even though the hardware and
  # ALSA device are otherwise correct. Keep this hardware-specific and harmless
  # on machines where the control does not exist.
  set_amixer_control_if_present "$card" 'Mic',0 127
  set_amixer_control_if_present "$card" 'Mic',1 127
  set_amixer_control_if_present "$card" 'UMC1820 Output',0 127
  set_amixer_control_if_present "$card" 'UMC1820 Output',1 127

  if amixer -c "$card" sget 'UMC1820 Clock Selector Clock Source' >/dev/null 2>&1; then
    amixer -c "$card" sset 'UMC1820 Clock Selector Clock Source' 'Internal Clock' >/dev/null 2>&1 || true
  fi

  channels="$(timeout 2s arecord -D "hw:${card},0" --dump-hw-params -f S32_LE -r 48000 -c 18 /tmp/nirucon-umc1820-check.wav 2>&1 \
    | awk -F': ' '/^CHANNELS:/ {print $2; exit}' \
    | tr -d '[]' || true)"

  if [[ "$channels" == "18" ]]; then
    ok "UMC1820 capture check: 18 channels available at S32_LE/48 kHz."
  else
    warn "Could not verify 18 UMC1820 capture channels right now. This can happen if the device is busy."
  fi

  # Deliberately do NOT set UMC1820 as PipeWire default here. On some machines
  # WirePlumber/PipeWire will then open the UMC1820 at login and block REAPER
  # when REAPER uses direct/exclusive ALSA. Keep system audio policy separate from
  # the hardware mixer safety initialization.

  sudo alsactl store "$card" >/dev/null 2>&1 || sudo alsactl store >/dev/null 2>&1 || \
    warn "Could not store ALSA mixer state. You can run manually: sudo alsactl store"

  ok "UMC1820 ALSA mixer defaults applied and stored."
}


optimize_reaper_for_umc1820_if_present() {
  local card ini
  card="$(find_alsa_card_by_name 'UMC1820|BEHRINGER.*UMC1820' || true)"
  ...
  [[ -n "${card:-}" ]] || return 0
  [[ -f "$ini" ]] || {
    note "REAPER config not found yet. UMC1820 REAPER defaults will be available through reaper-umc1820-alsa.sh."
    return 0
  }

  phase "Optimizing existing REAPER config for UMC1820 ALSA"
  backup_user_file "$ini"

  # Set only the narrow, hardware-specific ALSA fields we have verified on Studio/Othala.
  if grep -q '^alsa_indev=' "$ini"; then
    sed -i 's|^alsa_indev=.*|alsa_indev=hw:UMC1820|' "$ini"
  else
    sed -i '/^\[REAPER\]/a alsa_indev=hw:UMC1820' "$ini"
  fi

  if grep -q '^alsa_outdev=' "$ini"; then
    sed -i 's|^alsa_outdev=.*|alsa_outdev=hw:UMC1820|' "$ini"
  else
    sed -i '/^\[REAPER\]/a alsa_outdev=hw:UMC1820' "$ini"
  fi

  if grep -q '^alsa_rtprio=' "$ini"; then
    sed -i 's|^alsa_rtprio=.*|alsa_rtprio=10|' "$ini"
  else
    sed -i '/^\[REAPER\]/a alsa_rtprio=10' "$ini"
  fi

  ok "REAPER UMC1820 ALSA defaults applied: hw:UMC1820, alsa_rtprio=10."
}
...
echo
echo "== UMC1820 mixer, if present =="
if grep -qi "UMC1820" /proc/asound/cards 2>/dev/null; then
  card="$(awk 'BEGIN{IGNORECASE=1} /^[[:space:]]*[0-9]+[[:space:]]+\[/ {c=$1; gsub(/[^0-9]/,"",c); line=$0; getline d; if (line ~ /UMC1820/ || d ~ /UMC1820/) {print c; exit}}' /proc/asound/cards)"
  amixer -c "$card" sget 'Mic',0 2>/dev/null || true
  amixer -c "$card" sget 'Mic',1 2>/dev/null || true
  amixer -c "$card" sget 'UMC1820 Clock Selector Clock Source' 2>/dev/null || true
else
  echo "UMC1820 not detected"
fi
...
echo "   Prefer PipeWire/JACK for mixed desktop + DAW use."
echo "   Use ALSA hw:UMC1820 only for exclusive low-latency sessions."
echo "   For ALSA-exclusive sessions use: reaper-alsa-exclusive.sh"
EOF
...
EOF
  ...
  cat > "$LOCAL_BIN/umc1820-init.sh" <<'EOF'
#!/usr/bin/env bash
set -euo pipefail

card="$(awk 'BEGIN{IGNORECASE=1} /^[[:space:]]*[0-9]+[[:space:]]+\[/ {c=$1; gsub(/[^0-9]/,"",c); line=$0; getline d; if (line ~ /UMC1820/ || d ~ /UMC1820/) {print c; exit}}' /proc/asound/cards 2>/dev/null || true)"
...
echo "UMC1820 detected as ALSA card $card"
for ctl in "Mic,0" "Mic,1" "UMC1820 Output,0" "UMC1820 Output,1"; do
  name="${ctl%,*}"
  idx="${ctl##*,}"
  if amixer -c "$card" sget "$name",$idx >/dev/null 2>&1; then
    echo "Setting $name,$idx to 127"
    amixer -c "$card" sset "$name",$idx 127 >/dev/null 2>&1 || true
  fi
done
...
sudo alsactl store "$card" >/dev/null 2>&1 || sudo alsactl store >/dev/null 2>&1 || true

echo "Done. Recommended REAPER setting: Audio system ALSA, hw:UMC1820, 48 kHz, 128 samples, 3 periods."
EOF

  cat > "$LOCAL_BIN/reaper-umc1820-alsa.sh" <<'EOF'
#!/usr/bin/env bash
set -euo pipefail
...
if ! grep -qi "UMC1820" /proc/asound/cards 2>/dev/null; then
  echo "UMC1820 not detected. No REAPER changes made."
  exit 0
fi

if [[ ! -f "$ini" ]]; then
  echo "REAPER config not found: $ini"
  echo "Start REAPER once, close it, then run this script again."
  exit 0
fi

cp -a "$ini" "$ini.bak.$(date +%Y%m%d-%H%M%S)"

set_or_add_reaper_key() {
  local key="$1" value="$2"
  if grep -q "^${key}=" "$ini"; then
    sed -i "s|^${key}=.*|${key}=${value}|" "$ini"
  else
    sed -i "/^\[REAPER\]/a ${key}=${value}" "$ini"
  fi
}

set_or_add_reaper_key alsa_indev hw:UMC1820
set_or_add_reaper_key alsa_outdev hw:UMC1820
set_or_add_reaper_key alsa_rtprio 10

printf '%s\n' \
  "REAPER UMC1820 ALSA settings applied." \
  "Use in REAPER: Audio system ALSA, hw:UMC1820, 48 kHz, 128 samples, 3 periods."
EOF

  cat > "$LOCAL_BIN/reaper-alsa-exclusive.sh" <<'EOF'
#!/usr/bin/env bash
set -euo pipefail

# Run REAPER with direct/exclusive ALSA access.
# This is useful for UMC1820 low-latency recording when PipeWire/WirePlumber
# has opened the interface and blocks hw:UMC1820. PipeWire sockets are stopped
# temporarily and restored after REAPER exits, so desktop audio works again.

REAPER_BIN="${REAPER_BIN:-reaper}"
...
start_pipewire() {
  ...
  systemctl --user start wireplumber pipewire pipewire-pulse 2>/dev/null || true
}

trap start_pipewire EXIT

if grep -qi "UMC1820" /proc/asound/cards 2>/dev/null; then
  command -v umc1820-init.sh >/dev/null 2>&1 && umc1820-init.sh || true
fi
...
EOF
  ...
  cat > "$LOCAL_BIN/audio-pipewire-restart.sh" <<'EOF'
#!/usr/bin/env bash
set -euo pipefail
...
EOF
  ...
  cat > "$LOCAL_BIN/audio-who-uses-umc1820.sh" <<'EOF'
#!/usr/bin/env bash
set -euo pipefail
...
echo "UMC1820 ALSA card: $card"
ls -l /dev/snd/pcmC${card}D0* 2>/dev/null || true
echo
sudo fuser -v /dev/snd/pcmC${card}D0p /dev/snd/pcmC${card}D0c 2>/dev/null || true
echo
lsof /dev/snd/pcmC${card}D0* 2>/dev/null || true
EOF

  cat > "$LOCAL_BIN/daw-doctor.sh" <<'EOF'
#!/usr/bin/env bash
set -euo pipefail
...
find_umc_card() {
  awk 'BEGIN{IGNORECASE=1} /^[[:space:]]*[0-9]+[[:space:]]+\[/ {c=$1; gsub(/[^0-9]/,"",c); line=$0; getline d; if (line ~ /UMC1820/ || d ~ /UMC1820/) {print c; exit}}' /proc/asound/cards 2>/dev/null || true
}

echo "== NIRUCON DAW Doctor =="
echo
...
echo "rtprio:  $rt"
echo "memlock: $mem"
[[ "$mem" == "unlimited" ]] && ok "memlock is unlimited" || warn "memlock is not unlimited"
[[ "${rt:-0}" -ge 90 ]] 2>/dev/null && ok "rtprio is high enough" || warn "rtprio should be 90+ after relog/reboot"
echo

echo "== CPU governor =="
govs="$(cat /sys/devices/system/cpu/cpu*/cpufreq/scaling_governor 2>/dev/null | sort -u || true)"
echo "${govs:-unknown}"
echo "$govs" | grep -qx 'performance' && ok "CPU governor is performance" || warn "Use audio-performance.sh before recording"
echo

echo "== Services =="
for svc in pipewire pipewire-pulse wireplumber; do
  if systemctl --user is-active --quiet "$svc" 2>/dev/null; then ok "$svc active"; else warn "$svc not active"; fi
done
if systemctl is-active --quiet irqbalance 2>/dev/null; then ok "irqbalance active"; else warn "irqbalance not active"; fi
echo

echo "== Kernel RT runtime =="
...
if [[ -n "${card:-}" ]]; then
  ok "UMC1820 detected as ALSA card $card"
  amixer -c "$card" sget 'Mic',0 2>/dev/null | sed -n '1,22p' || true
  amixer -c "$card" sget 'Mic',1 2>/dev/null || true
  amixer -c "$card" sget 'UMC1820 Clock Selector Clock Source' 2>/dev/null || true
  if timeout 2s arecord -D "hw:${card},0" --dump-hw-params -f S32_LE -r 48000 -c 18 /tmp/daw-doctor-umc1820.wav >/tmp/daw-doctor-umc1820.log 2>&1; then
    true
  fi
  if grep -q '^CHANNELS: 18$' /tmp/daw-doctor-umc1820.log 2>/dev/null; then
    ok "UMC1820 exposes 18 capture channels at S32_LE/48 kHz"
  else
    warn "Could not verify 18 capture channels now; device may be busy"
    sed -n '1,30p' /tmp/daw-doctor-umc1820.log 2>/dev/null || true
  fi
else
  info "UMC1820 not connected/detected. Skipping hardware-specific checks."
fi
echo
...
ini="$HOME/.config/REAPER/reaper.ini"
if [[ -f "$ini" ]]; then
  grep -E '^(alsa_indev|alsa_outdev|alsa_rtprio)=' "$ini" || true
else
  info "REAPER config not found yet"
fi
if pgrep -x reaper >/dev/null 2>&1; then
  lsof /dev/snd/* 2>/dev/null | grep -i reaper || true
else
  info "REAPER is not running"
fi
echo

echo "== UMC1820 ownership / exclusive ALSA check =="
if [[ -n "${card:-}" ]]; then
  fuser -v /dev/snd/pcmC${card}D0p /dev/snd/pcmC${card}D0c 2>/dev/null || ok "UMC1820 PCM devices look free"
  if fuser /dev/snd/pcmC${card}D0p 2>/dev/null | grep -q .; then
    warn "UMC1820 playback is already open. Direct ALSA REAPER may be blocked. Use audio-who-uses-umc1820.sh."
  fi
else
  info "UMC1820 not detected"
fi
echo

echo "== Recent audio warnings =="
journalctl -b --no-pager 2>/dev/null | grep -Ei 'snd|alsa|usb|xrun|overrun|underrun|pipewire' | tail -40 || true
EOF
...
EOF
  ...
  chmod +x "$LOCAL_BIN/audio-performance.sh" "$LOCAL_BIN/audio-balanced.sh" "$LOCAL_BIN/audio-status.sh" "$LOCAL_BIN/reaper-audio-check.sh" "$LOCAL_BIN/yabridge-sync.sh" "$LOCAL_BIN/umc1820-init.sh" "$LOCAL_BIN/reaper-umc1820-alsa.sh" "$LOCAL_BIN/reaper-alsa-exclusive.sh" "$LOCAL_BIN/audio-pipewire-restart.sh" "$LOCAL_BIN/audio-who-uses-umc1820.sh" "$LOCAL_BIN/daw-doctor.sh"

  ok "Audio helper scripts created."

  note "Existing REAPER audio backend is not forced. Use reaper-umc1820-alsa.sh or reaper-alsa-exclusive.sh when desired."
...
if [[ -z "${DISPLAY:-}" ]]; then
  echo "No DISPLAY detected. Monitor status requires a local X11/dwm session."
  exit 0
fi
...
if [[ -z "${DISPLAY:-}" ]]; then
  echo "No DISPLAY detected. arandr requires a local X11/dwm session."
  exit 1
fi
...
if [[ -z "${DISPLAY:-}" ]]; then
  echo "No DISPLAY detected. Saving monitor profiles requires a local X11/dwm session."
  exit 1
fi
...
if [[ -z "${DISPLAY:-}" ]]; then
  echo "No DISPLAY detected. Skipping monitor apply."
  exit 0
fi
...
if [[ -z "${DISPLAY:-}" ]]; then
  echo "No DISPLAY detected. Rotating monitors requires a local X11/dwm session."
  exit 1
fi
...
sudo mkdir -p "$(dirname "$SESSION_DESKTOP")"
...
echo "  daw-doctor:      $(command -v daw-doctor.sh || echo optional/missing)"
...
if [[ "$INSTALL_AUDIO_TOOLS" -eq 1 ]]; then
  ...
  echo "  daw-doctor.sh             # full DAW health check: realtime, UMC1820, ALSA/PipeWire, REAPER"
  echo "  audio-performance.sh      # use before recording/mixing"
  echo "  audio-balanced.sh         # use after audio work"
  echo "  audio-pipewire-restart.sh # restore PipeWire desktop audio after exclusive ALSA work"
  echo "  audio-who-uses-umc1820.sh # show which process currently holds the UMC1820"
  echo "  yabridge-sync.sh          # sync Windows VST paths when yabridge is installed"
  echo "  umc1820-init.sh           # reset/store UMC1820 ALSA mixer defaults if the card is connected"
  echo "  reaper-umc1820-alsa.sh    # set existing REAPER config to ALSA hw:UMC1820 + rtprio 10"
  echo "  reaper-alsa-exclusive.sh  # stop PipeWire temporarily, run REAPER, restore PipeWire after exit"
...
echo "  Recommended first test: run daw-doctor.sh."
echo "  UMC1820 options:"
echo "    - PipeWire/JACK for mixed desktop + DAW use."
echo "    - reaper-alsa-exclusive.sh for direct ALSA low-latency sessions."
echo
ok "Done."
